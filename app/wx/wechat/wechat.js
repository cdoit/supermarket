    ...
    if (
      ...
      that.requestGet(url).then(function(data) {
        ...
        if (data.indexOf("errcode") < 0) {
          ...
          //更新本地存储的
          console.log("write toaken");
          fs.writeFile(
            "./app/wx/wechat/access_token.json",
          ...
          switch (result.Event.toLowerCase()) {
            case "subscribe":
              var contentArr = [
                {
                  Title: "欢迎逛别墅超市",
                  Description: "在下方点击“我的”，预约和业务咨询",
                  PicUrl: "http://www.faruxue1688.com/images/Abuild.jpg",
                  Url:
                    "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx459b286ba935a855&redirect_uri=http://www.faruxue1688.com/wx/marketingregister&response_type=code&scope=snsapi_userinfo&state=1&connect_redirect=1#wechat_redirect"
                }
              ];
              //回复图文消息
              reportMsg = msg.graphicMsg(fromUser, toUser, contentArr);
              console.log("username:" + result.FromUserName);
              console.log("evntkey:" + result.EventKey);
              var rolekey = result.EventKey;
              var raks = rolekey.split("_");
              //当前账号的归属
              var sponsor = parseInt(raks[1]);
              var roleid = 0;
              //当前归属为9527时，说明是销售
              if (sponsor == 9527) {
                //销售角色
          ...
          if (result.MsgType.toLowerCase() === "text") {
            ...
            switch (result.Content) {
              ...
              case "文章":
                ...
                // reportMsg = msg.graphicMsg(fromUser,toUser,contentArr);
                var content =
                  "欢迎关注 叮当快建 公众号，叮当快建是一个提供专业的楼板加建产品服务的网站：\n";
                content += "1.你是谁\n";
                content += "2.关于Node.js\n";
